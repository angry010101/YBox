...
import com.yakymovych.simon.everywhere.*
import com.yakymovych.simon.everywhere.custom.AutoFitTextureView
import com.yakymovych.simon.everywhere.ui.BaseFragment
import com.yakymovych.simon.everywhere.ui.dialogs.ConfirmationDialog
import com.yakymovych.simon.everywhere.ui.dialogs.ErrorDialog
...
class Camera2VideoFragment : BaseFragment(), View.OnClickListener,
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.video -> if (isRecordingVideo) stopRecordingVideo() else createConnection()
            R.id.info -> {
                if (activity != null) {
    ...
    private fun setUpMediaRecorder(socket: Socket) {
    ...
    private fun createConnection(){
        viewModel.createConnection()?.subscribe({
            this.startRecordingVideo(viewModel.repository.socket!!)
        },{
            Handler().post {
                Toast.makeText(this.context,"ERROR HAPPENED",Toast.LENGTH_SHORT).show()
            }
        })

    }

    private fun startRecordingVideo(socket: Socket) {
        if (cameraDevice == null || !textureView.isAvailable) return
